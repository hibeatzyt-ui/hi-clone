...
class HiCloneGame {
  ...
  load(chartData) {
    ...
    this._linkHoldChains();
  }

  // A "rail" made of consecutive hold segments (e.g. h1>2 then h2>1, back
  // to back with no gap) can't be dragged between keys on a keyboard — the
  // player holds one physical key the whole time. So every segment in a
  // chain remembers the FIRST segment's startLane as chainStartLane; that's
  // the key that has to stay held to auto-continue through later segments,
  // regardless of what each segment's own start/end lanes say visually.
  _linkHoldChains() {
    let chainStartLane = null;
    let prevEnd = null;
    for (const n of this.notes) {
      if (n.kind !== 'hold') { prevEnd = null; continue; }
      if (prevEnd !== null && Math.abs(n.t - prevEnd) < 0.02) {
        n.chainStartLane = chainStartLane;
      } else {
        chainStartLane = n.startLane;
        n.chainStartLane = chainStartLane;
      }
      prevEnd = n.t + n.dur;
    }
  ...
  handleLaneUp(lane) {
    ...
    // resolve any active hold whose governing physical key is this lane
    // (for a chained rail that's chainStartLane, not the segment's own startLane)
    for (const [id, h] of this.activeHolds.entries()) {
      const note = this.notes.find(n => n.id === id);
      if (note && h.physicalLane === lane && note.holdState === 'active') {
  ...
  _tryJudgeLane(lane, t) {
    ...
    if (best.kind === 'hold') {
      ...
      this.activeHolds.set(best.id, { pressStart: t, physicalLane: lane });
      return;
    }
  ...
  _loop() {
    ...
    // a chained rail (consecutive hold segments) needs to keep activating
    // while its chain's original key is already held down — there's no
    // fresh keydown between segments, and segments can't be "dragged to"
    // since keyboard input can't slide between keys
    for (const n of this.notes) {
      if (n.kind === 'hold' && n.holdState === 'pending' && !n.judged &&
          this.pressed.has(n.chainStartLane) && t >= n.t) {
        n.holdState = 'active';
        this.activeHolds.set(n.id, { pressStart: t, physicalLane: n.chainStartLane });
      }
    }
