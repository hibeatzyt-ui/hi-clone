...
// Song folders under Hi Beatz/ are discovered automatically (via the GitHub
// API when running on GitHub Pages, or Hi Beatz/songs.json as a fallback for
// local testing). info.json is optional — if it's missing, a title is
// derived straight from the folder name. See Hi Beatz/README.md for details.
...
function inferDifficulty(folder, info) {
  ...
  const m = /_([A-Za-z]+)$/.exec(folder);
  if (m && DIFF_SUFFIX[m[1].toUpperCase()]) return DIFF_SUFFIX[m[1].toUpperCase()];
  return 'Standard';
}

// Turns "shape_of_you_ST" or "shape-of-you-ST" into "Shape Of You" — used
// as the title whenever a song doesn't have its own info.json.
function humanizeFolderName(folder) {
  const base = folder.replace(/[_-](ST|GOLD|DIAMOND)$/i, '');
  return base
    .replace(/[_-]+/g, ' ')
    .replace(/\s+/g, ' ')
    .trim()
    .replace(/\b\w/g, c => c.toUpperCase()) || folder;
}

// When running on GitHub Pages, {user}.github.io/{repo}/... tells us the
// owner/repo directly from the URL — no configuration needed.
...
async function discoverSongFolders() {
  const gh = detectGithubPagesRepo();
  if (gh) {
    try {
      const url = `https://api.github.com/repos/${gh.owner}/${gh.repo}/contents/${encodeURIComponent(LIBRARY_ROOT)}`;
      const res = await fetch(url, { cache: 'no-store' });
      if (res.ok) {
        const items = await res.json();
        if (Array.isArray(items)) {
          const folders = items.filter(i => i.type === 'dir' && !i.name.startsWith('_')).map(i => i.name);
          if (folders.length) return { folders, source: 'github' };
        }
      }
    } catch (err) {
      console.warn('GitHub folder auto-discovery failed, falling back to songs.json', err);
    }
  }

  // fallback: Hi Beatz/songs.json (also what local testing without GitHub Pages uses)
  try {
    const res = await fetch(`${LIBRARY_ROOT}/songs.json`, { cache: 'no-store' });
    if (res.ok) {
      const folders = await res.json();
      if (Array.isArray(folders)) return { folders, source: 'songs.json' };
    }
  } catch (err) {}
...
async function loadLibrary() {
  ...
  const { folders, source } = await discoverSongFolders();
  ...
  for (const folder of folders) {
    ...
    try {
      ...
      // info.json is optional — a missing/unreadable one just falls back below
    }

    const difficulty = inferDifficulty(folder, info);
    const entry = {
      folder,
      title: info.title || humanizeFolderName(folder),
  ...
  if (songGroups.length) {
    statusEl.textContent = `${songGroups.length} song${songGroups.length === 1 ? '' : 's'} loaded from Hi Beatz`;
  } else if (source === 'none') {
    statusEl.textContent = 'Could not find your Hi Beatz folder — see the README to set up your library.';
  } else {
    statusEl.textContent = 'Your Hi Beatz folder is empty — add a song to get started.';
  }
  renderSongList();
}
...
function renderSongList() {
  ...
  if (!songGroups.length) {
    list.innerHTML = `<div class="empty-state">
      ...
      <code>chart.chart</code> and <code>audio</code> inside, upload it, and
      it'll show up here automatically — the title comes straight from the
      folder name. Add an <code>info.json</code> (or use "Add a Song" from
      the home screen to generate one) only if you want a custom title,
      artist, or BPM.
    </div>`;
    return;
  }
