...
— there's no server, no database, nothing to install. Every song is one
folder in here. **Just add the folder and upload it — Hi Clone finds it
automatically** (via GitHub's own API when running on GitHub Pages). You
don't need to maintain any list of songs, and you don't need to write
`info.json` either unless you want to customize the title/artist/BPM.
...
Songs are grouped in-game by title + artist, so `7rings_ST` and
`7rings_DIAMOND` show up as one song card in Song Select with two
selectable difficulties.

**The title shown in-game comes straight from the folder name if you don't
add an `info.json`** — `shape_of_you_ST` becomes "Shape Of You" automatically
(underscores/dashes become spaces, each word capitalized). So the fastest
way to add a song is: name the folder well, skip `info.json` entirely.
...
7rings_DIAMOND/
  ...
  info.json      optional — only needed to override title/artist/BPM
```
...
Only add this if the folder-name-derived title isn't good enough, or you
want a real artist name instead of the "Unknown Artist" default, or a
specific BPM:
...
JSON for you (and shows you the folder name to create) — but again, this
...
Nothing to do here — just upload the folder. Hi Clone automatically lists
every subfolder in `Hi Beatz/` straight from GitHub, so as soon as your
song's folder is pushed, it shows up in Song Select on next load.

(If you're testing locally without GitHub Pages — e.g. running
`python3 -m http.server` — GitHub's folder listing isn't available, so Hi
Clone falls back to reading `Hi Beatz/songs.json` instead. For local testing
only, add the folder name there: `["7rings_ST", "7rings_DIAMOND"]`.)

## 6. Converting from Moonscraper (.chart guitar files)
...
still need to drop in `audio.mp3` + `artwork.jpg` yourself — after that,
just upload the folder, no `songs.json` editing needed. Run with `-h` for
all options (custom section names, other instrument tracks like `Drums`, etc.).

## 7. A note on audio files
